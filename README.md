...
- 역할: 
  ...
  - 이다원 : 조장, 상품 등록/수정/삭제, 게시판 기능, 관리자 기능
  - 강치우 : 게시글 신고, 유저 신고
  - 김현진 : 상품 목록 페이지, 상품 상세 페이지, 바나나페이(I'M port API 이용)
  - 김효동 : 회원가입 페이지, 로그인 페이지, ID/PW 찾기, 채팅 페이지
  - 이수연 : 마이페이지(회원정보 조회/변경, 상품 목록, 관심목록 조회, 매너평가, 거래후기 작성/조회, 차단관리)
...
## 5. 핵심 트러블 슈팅
### 5.1. 상품 등록 유효성 검사
- input type = file에 multiple기능이 들어가면서 이미지의 등록과 수정, 삭제가 까다롭다는 생각을 많이 했습니다.
수업과는 달리 이미지의 order가 없어서 
