...
(() => {
    ...
    for (let i = 0; i < 5; i++) {
        if (preview[i] != undefined && preview[i].getAttribute("src") == "") {
            boardImg[i].remove();
            i--;
            imageCount--;
        } else {
        ...
        if (imageCount >= 5) {
            ...
            for (let i = 0; i < 5; i++) {
                ...
                //if (preview[i] != undefined && preview[i].getAttribute("src") == "") {
                    ...
                    imageCount = document.getElementsByClassName("preview").length;
                    // for (let i = 0; i < 5; i++) {
...
for (let i = 0; i < inputImage.length; i++) {
    ...
    inputImage[i].addEventListener("change", (e) => {
        ...
        if (e.target.files[0] != undefined) { // 선택된 파일이 있을 경우
            ...
            // 남은 boardImg요소의 순번 다시 부여
            for(let j = 0; j < boardImg.length; j++) {
                boardImg.firstChild.setAttribute("for", "img" + j);
                boardImg.lastChild.setAttribute("id", "img" + i);
            }
        }
    });
